...
export const getProfile = () => Store.get(Conf.PROFILE);
 ...
 */
export const getLanguage = (withDefault = true) => {
    const s = getLocale();
    if (!s) {
        if (withDefault) {
 ...
 */
export const set = (profile, permissions, locale) => {
    Store.set(Conf.PROFILE, profile);
    Store.set(Conf.PERMISSIONS, permissions);
    setLocale(locale);
};
export const setLocale = (locale) => Store.set(Conf.LOCALE, locale);
export const remove = () => {
